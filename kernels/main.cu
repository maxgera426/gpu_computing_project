...
__constant__ float d_P[12];  // Matrice P pour projection

__global__ void optimized_compute_kernel(
    // améliorer la computation speed
    // 1 : précalculer la matrice de projection
    // 2 : le calcul de float z coté cpu puis lookup table
    // Reference data
    int ref_width, int ref_height, unsigned char* ref_Y,
...
)
{
    ...
    // Utiliser la matrice d_P (3x4) directement
    float X_cam = d_P[0] * x + d_P[1] * y + d_P[2] * z + d_P[3];
    float Y_cam = d_P[4] * x + d_P[5] * y + d_P[6] * z + d_P[7];
    float Z_cam = d_P[8] * x + d_P[9] * y + d_P[10] * z + d_P[11];

    float x_proj = X_cam / Z_cam;
    float y_proj = Y_cam / Z_cam;
