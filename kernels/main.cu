...
)
{
    ...
    // Early exit if out of bounds for computation
    if (x >= ref_width || y >= ref_height || zi >= ZPlanes) {
        return;
    }
...
__global__ void grid3d_shared_both_kernel(
    // Reference data
    int ref_width, int ref_height, unsigned char* ref_Y,
    // Camera data
    int cam_width, int cam_height, unsigned char* cam_Y,
...
)
{
    ...
    // Early exit if out of global bounds for computation
    if (x >= ref_width || y >= ref_height || zi >= ZPlanes) {
        return;
    }

    // Calculate padding for window
    int padding = window / 2;
    ...
    // Declare shared memory for reference image and camera image
    // Dynamic shared memory layout: 
    // First part: reference image with padding
    // Second part: camera image projected area with padding
    extern __shared__ unsigned char shared_mem[];
    unsigned char* s_ref = shared_mem;
    unsigned char* s_cam = &shared_mem[smem_width * smem_height];

    // Local thread indices (for shared memory access)
    int tx = threadIdx.x;
    int ty = threadIdx.y;

    // Calculate block start coordinates in global memory for reference image
    int block_start_x = blockIdx.x * blockDim.x - padding;
    int block_start_y = blockIdx.y * blockDim.y - padding;

    /////////////////////////////////////////////////////////////
    // STEP 1: Load reference image into shared memory
    /////////////////////////////////////////////////////////////
    ...
    // Top-right corner
    if ((tx == blockDim.x - 1 || x == ref_width - 1) && ty == 0) {
        for (int j = 0; j < padding; j++) {
            for (int i = 1; i <= padding; i++) {
                int load_y = block_start_y + j;
                int load_x = x + i;
    ...
    /////////////////////////////////////////////////////////////
    // STEP 2: Calculate camera projections for this block
    /////////////////////////////////////////////////////////////
    ...
    // Check if projection is valid and round to nearest pixel
    x_proj = roundf(x_proj);
    y_proj = roundf(y_proj);

    // Store projected coordinates for later use
    int x_proj_i = (int)x_proj;
    int y_proj_i = (int)y_proj;

    // Check if projection is within camera bounds
    bool valid_proj = (x_proj_i >= 0 && x_proj_i < cam_width &&
        y_proj_i >= 0 && y_proj_i < cam_height);

    /////////////////////////////////////////////////////////////
    // STEP 3: Load camera image into shared memory
    /////////////////////////////////////////////////////////////

    __syncthreads();  // Ensure reference image loading is complete

    // Load primary camera position
    if (valid_proj) {
        s_cam[s_idx] = cam_Y[y_proj_i * cam_width + x_proj_i];
    }
    else {
        s_cam[s_idx] = 0; // Default value for invalid projections
    }

    // Each thread also loads surrounding camera pixels based on the window size
    // We need to calculate the corresponding camera coordinates for each thread's
    // position in the surrounding window

    // Top edge (first row of threads)
    if (ty == 0) {
        for (int i = 0; i < padding; i++) {
            int cam_y = y_proj_i - padding + i;

            // Clamp to valid camera image coordinates
            cam_y = max(0, min(cam_height - 1, cam_y));
            int s_y_idx = i * smem_width + (tx + padding);

            if (valid_proj) {
                s_cam[s_y_idx] = cam_Y[cam_y * cam_width + x_proj_i];
            }
            else {
                s_cam[s_y_idx] = 0;
            }
    ...
    // Bottom edge (last row of threads)
    if (ty == blockDim.y - 1 || y == ref_height - 1) {
        for (int i = 1; i <= padding; i++) {
            int cam_y = y_proj_i + i;
    ...
    // Left edge (first column of threads)
    if (tx == 0) {
        for (int i = 0; i < padding; i++) {
            int cam_x = x_proj_i - padding + i;

            // Clamp to valid camera image coordinates
            cam_x = max(0, min(cam_width - 1, cam_x));
            int s_x_idx = (ty + padding) * smem_width + i;

            if (valid_proj) {
                s_cam[s_x_idx] = cam_Y[y_proj_i * cam_width + cam_x];
            }
            else {
                s_cam[s_x_idx] = 0;
            }
    ...
    // Right edge (last column of threads)
    if (tx == blockDim.x - 1 || x == ref_width - 1) {
        for (int i = 1; i <= padding; i++) {
            int cam_x = x_proj_i + i;
    ...
    if (tx == 0 && ty == 0) {
        for (int j = 0; j < padding; j++) {
            for (int i = 0; i < padding; i++) {
                ...
                if (valid_proj) {
                    s_cam[s_corner_idx] = cam_Y[cam_y * cam_width + cam_x];
                }
                else {
                    s_cam[s_corner_idx] = 0;
                }
            }
        }
    }
    ...
    /////////////////////////////////////////////////////////////
    // STEP 4: Calculate cost using shared memory for both images
    /////////////////////////////////////////////////////////////

    float cost = 0.0f;
    float cc = 0.0f;

    // The cost calculation now uses both shared memory buffers
    for (int k = -padding; k <= padding; k++) {
        for (int l = -padding; l <= padding; l++) {
            // Access reference pixel from shared memory
            int s_y = ty + padding + k;
            int s_x = tx + padding + l;
            int s_idx = s_y * smem_width + s_x;

            unsigned char ref_val = s_ref[s_idx];
            unsigned char cam_val = s_cam[s_idx];

            // If valid projection (we already ensured bounds checking during loading)
            cost += fabsf((float)ref_val - (float)cam_val);
            cc += 1.0f;
        }
    }
...
std::vector<cv::Mat> sweeping_plane_grid3d_shared_both(cam const& ref, std::vector<cam> const& cam_vector, int window = 3) {
    int width = ref.width;
    int height = ref.height;
    int total_size = width * height;
    ...
    for (auto& cam : cam_vector) {
        ...
        // Calculate shared memory size for reference and camera images with padding
        int smem_width = blockDim.x + 2 * padding;
        int smem_height = blockDim.y + 2 * padding;
        size_t smem_size = 2 * smem_width * smem_height * sizeof(unsigned char); // Double for ref and cam
        ...
        // Launch kernel with 3D grid, 2D blocks, and shared memory for both images
        grid3d_shared_both_kernel << <gridDim, blockDim, smem_size >> > (
            width, height, d_ref_Y,
            cam.width, cam.height, d_cam_Y,
            d_cost_cube, window,
            static_cast<float>(ZNear), static_cast<float>(ZFar), ZPlanes
